@extends('layouts.app')
@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-[40px] sm:pt-[60px] lg:pt-[80px] flex flex-col lg:flex-row gap-8">
    <!-- Боковая панель с фильтрами (остается слева) -->
    <div class="w-full lg:w-auto lg:min-w-[350px]">
        <form method="GET" action="{{ route('containers') }}" id="filterForm">
            <div class="flex justify-center items-left flex-col">
                ...
                <button type="button" id="resetFilters" class="flex flex-row bg-[#71C2FF] text-white rounded-[20px] p-[15px] text-[18px] gap-6 items-center justify-center transition duration-150 hover:bg-[#179BFF] group w-full sm:w-auto">
                    Сбросить фильтры
    ...
    <div class="w-full">
        <div class="flex justify-end mb-4">
            <form method="GET" action="{{ route('containers') }}" class="relative w-full sm:w-auto">
                @foreach($queryParams as $key => $value)
                    ...
                    @endif
                @endforeach
                <select name="sort" id="sortSelectForm" class="appearance-none bg-[#F3F4F6] border border-[#179BFF]/30 rounded-[15px] px-6 py-3 pr-10 text-[#595959] text-16px cursor-pointer focus:outline-none focus:border-[#179BFF] transition-colors duration-150 w-full sm:w-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-[30px] mt-[20px]">
            @foreach ($per as $pere)
            <div class="bg-[#F3F4F6] rounded-[15px] border border-[#595959]/10 shadow-[4px_4px_10px_-5px_#71C2FF] transition duration-150 hover:border-[#179BFF]/30 hover:shadow-[7px_7px_25px_-5px_#71C2FF] h-full">
                <div class="overflow-hidden rounded-t-[14px]">
                    <img class="w-full h-[200px] sm:h-[220px] object-cover transition duration-300 hover:scale-105" src="{{  asset('$pere->image')  }}" alt="Контейнер №{{ $pere->number }}">
                </div>
                <div class="p-[20px] flex flex-col items-start">
                    ...
                    <p class="text-[#595959] text-16px mb-[15px] line-clamp-1">Комплекс "{{ $pere->residentialComplex->title }}"</p>
                    ...
                    <h5 class="text-black text-24px mb-[20px]">{{ number_format($pere->daily_price, 0, ',', ' ') }} ₽ / д.</h5>
                    <a class="w-full" href="{{ route('container_card', $pere->id) }}">
                        <button class="py-2.5 text-white text-2xl w-full bg-[#71C2FF] rounded-lg transition duration-150 hover:bg-[#179BFF] flex flex-row items-center justify-center group" type="button">
