<footer class="bg-white text-white mt-auto">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-[200px]">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            <!-- Логотип и соцсети -->
            <div class="text-center sm:text-left">
                <div class="flex flex-col sm:flex-row items-center sm:items-start mb-6">
                    <img src="{{  asset('img/Logo.svg')  }}" width="80px" class="mb-4 sm:mb-0">
                    <h4 class="text-32px text-black font-semibold sm:pl-4">ХоумБокс</h4>
                </div>
                <p class="text-[#595959] text-14px mb-6 hover:text-[#231F20] text-center sm:text-left">&copy; {{ date('Y') }} ХоумБокс. Все права защищены.</p>
                <div class="flex flex-row justify-center sm:justify-start gap-x-4">
                    <a href="https://vk.ru/id415635857" class="group">
                        <svg class="w-10 h-10 fill-current text-[#595959] hover:text-[#0077FF] transition duration-150" viewBox="0 0 100 100">
                        <path d="M50 4C24.636 4 4 24.636 4 50C4 75.364 24.636 96 50 96C75.366 96 96 75.364 96 50C96 24.636 75.366 4 50 4ZM69.946 59.07C74.42 63.042 75.35 64.46 75.502 64.702C77.356 67.642 73.446 68 73.446 68H65.206C65.206 68 63.2 68.022 61.486 66.886C58.692 65.056 55.766 61.508 53.71 62.124C51.984 62.64 52 64.972 52 66.966C52 67.68 51.386 68 50 68C48.614 68 48.038 68 47.424 68C42.91 68 38.012 66.48 33.126 61.374C26.214 54.156 20.152 39.616 20.152 39.616C20.152 39.616 19.794 38.884 20.184 38.438C20.626 37.938 21.828 38.002 21.828 38.002L29.818 38C29.818 38 30.57 38.142 31.11 38.522C31.556 38.834 31.804 39.43 31.804 39.43C31.804 39.43 33.146 43.862 34.856 46.688C38.196 52.204 39.75 52.344 40.884 51.75C42.54 50.89 42 45.026 42 45.026C42 45.026 42.074 42.508 41.21 41.386C40.544 40.518 39.27 40.056 38.714 39.984C38.264 39.926 39.016 39.138 39.974 38.688C41.254 38.118 42.996 37.972 46 38C48.338 38.022 49.012 38.162 49.924 38.372C52.682 39.008 52 40.686 52 46.578C52 48.466 51.74 51.12 53.164 52C53.778 52.38 55.882 52.844 59.626 46.764C61.404 43.88 62.818 39.096 62.818 39.096C62.818 39.096 63.11 38.57 63.564 38.31C64.028 38.044 64.014 38.05 64.65 38.05C65.286 38.05 71.664 38 73.064 38C74.462 38 75.774 37.984 76 38.804C76.324 39.982 74.968 44.018 71.532 48.398C65.886 55.586 65.26 54.914 69.946 59.07Z"/>
                        </svg>
                    </a>
                    <a href="https://t.me/Aspekttt" class="group">
                        <svg class="w-10 h-10 fill-current text-[#595959] hover:text-[#27A7E7] transition duration-150" viewBox="0 0 100 100">
                        <path d="M50 4C75.406 4 96 24.594 96 50C96 75.406 75.406 96 50 96C24.594 96 4 75.406 4 50C4 24.594 24.594 4 50 4ZM65.868 68.75C66.714 66.154 70.678 40.282 71.168 35.184C71.316 33.64 70.828 32.614 69.872 32.156C68.716 31.6 67.004 31.878 65.018 32.594C62.294 33.576 27.47 48.362 25.458 49.218C23.55 50.028 21.746 50.912 21.746 52.192C21.746 53.092 22.28 53.598 23.752 54.124C25.284 54.67 29.142 55.84 31.42 56.468C33.614 57.074 36.112 56.548 37.512 55.678C38.996 54.756 56.122 43.296 57.352 42.292C58.58 41.288 59.56 42.574 58.556 43.58C57.552 44.584 45.796 55.994 44.246 57.574C42.364 59.492 43.7 61.48 44.962 62.276C46.404 63.184 56.774 70.14 58.336 71.256C59.898 72.372 61.482 72.878 62.932 72.878C64.382 72.878 65.146 70.968 65.868 68.75Z"/>
                        </svg>
                    </a>
                    <a href="https://api.whatsapp.com/send?phone=79507980466" class="group">
                        <svg class="w-10 h-10 fill-current text-[#595959] hover:text-[#2CB742] transition duration-150" viewBox="0 0 100 100">
                        <path d="M49.9998 4C24.6358 4 3.99981 24.636 3.99981 50C3.99981 57.92 6.04581 65.708 9.92581 72.58L4.07381 93.46C3.88181 94.146 4.06781 94.882 4.56381 95.392C4.94581 95.786 5.46581 96 5.99981 96C6.15981 96 6.32181 95.98 6.47981 95.942L28.2718 90.544C34.9258 94.116 42.4198 96 49.9998 96C75.3638 96 95.9998 75.364 95.9998 50C95.9998 24.636 75.3638 4 49.9998 4ZM73.1398 66.232C72.1558 68.956 67.4358 71.442 65.1678 71.776C63.1318 72.074 60.5558 72.202 57.7278 71.314C56.0138 70.774 53.8138 70.058 50.9958 68.856C39.1498 63.804 31.4138 52.026 30.8218 51.248C30.2318 50.47 25.9998 44.926 25.9998 39.188C25.9998 33.45 29.0498 30.628 30.1338 29.46C31.2178 28.292 32.4958 28 33.2838 28C34.0718 28 34.8578 28.01 35.5478 28.042C36.2738 28.078 37.2478 27.768 38.2058 30.044C39.1898 32.38 41.5518 38.118 41.8438 38.704C42.1398 39.288 42.3358 39.97 41.9438 40.748C41.5518 41.526 41.3558 42.012 40.7638 42.694C40.1718 43.376 39.5238 44.214 38.9918 44.738C38.3998 45.32 37.7858 45.95 38.4738 47.118C39.1618 48.286 41.5318 52.104 45.0438 55.196C49.5538 59.168 53.3598 60.4 54.5398 60.984C55.7198 61.568 56.4098 61.47 57.0978 60.692C57.7858 59.912 60.0498 57.286 60.8358 56.12C61.6218 54.954 62.4098 55.146 63.4938 55.536C64.5778 55.924 70.3838 58.744 71.5638 59.328C72.7438 59.912 73.5318 60.204 73.8278 60.69C74.1238 61.174 74.1238 63.51 73.1398 66.232Z"/>
                        </svg>
                    </a>
                </div>
            </div>
            <!-- Навигация -->
            <div class="text-center sm:text-left">
                <h4 class="text-lg text-black font-semibold mb-4">Навигация</h4>
                <ul class="space-y-3">
                    <li><a href="{{ route('about') }}" class="text-[#595959] hover:text-[#231F20] transition duration-150">О нас</a></li>
                    <li><a href="{{ route('complexes') }}" class="text-[#595959] hover:text-[#231F20] transition duration-150">Комплексы</a></li>
                    <li><a href="{{ route('containers') }}" class="text-[#595959] hover:text-[#231F20] transition duration-150">Контейнеры</a></li>
                    <li><a href="{{ route('profile.edit') }}" class="text-[#595959] hover:text-[#231F20] transition duration-150">Профиль</a></li>
                    <li><a href="{{ route('profile.bookings') }}" class="text-[#595959] hover:text-[#231F20] transition duration-150">Мои бронирования</a></li>
                    <li><a href="{{ route('profile.booking-history') }}" class="text-[#595959] hover:text-[#231F20] transition duration-150">История аренд</a></li>
                </ul>
            </div>
            <!-- Контакты -->
            <div class="text-center sm:text-left">
                <h4 class="text-lg text-black font-semibold mb-4">Контакты</h4>
                <ul class="space-y-3">
                    <li class="text-[#595959] hover:text-[#231F20] transition duration-150 break-all">mailmail@mail.mail</li>
                    <li class="text-[#595959] hover:text-[#231F20] transition duration-150">+8 800 555 35 35</li>
                    <li class="text-[#595959] hover:text-[#231F20] transition duration-150">ул. Пушкина д. 117</li>
                </ul>
            </div>
            <!-- Правовая информация -->
            <div class="text-center sm:text-left">
                <h4 class="text-lg text-black font-semibold mb-4">Правовая информация</h4>
                <ul class="space-y-3">
                    <li class="text-[#595959] hover:text-[#231F20] transition duration-150 cursor-pointer">Политика конфиденциальности</li>
                    <li class="text-[#595959] hover:text-[#231F20] transition duration-150 cursor-pointer">Пользовательское соглашение</li>
                </ul>
            </div>
        </div>
    </div>
</footer>
