...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-[40px] sm:pt-[60px] lg:pt-[80px]">
    <h3 class="text-[20px] sm:text-[22px] mb-[30px] sm:mb-[40px] lg:mb-[50px]">Текущие бронирования</h3>
                        ...
                        <div>
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-0">
                                <div class="flex flex-row flex-wrap items-center gap-2 sm:gap-3">
                                    <h4 class="border-2 rounded-[20px] px-3 sm:px-4 py-1 mb-2 text-sm sm:text-base
                                        @if($booking->status == 'Новая')
                                            text-[#58ABEA] border-[#58ABEA]
                                        @elseif($booking->status == 'Подтверждена')
                                            text-[#00C851] border-[#00A919]
                                        @endif
                                    ">{{ $booking->status }}</h4>
                                    <h6 class="text-[#595959] text-base sm:text-lg mb-2">Контейнер №{{ $booking->container->number }}</h6>
                                </div>
                                <div>
                                    <form action="{{ route('bookings.cancel', $booking->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-white bg-[#FF6767] border-2 border-[#FF6767] rounded-[20px] px-4 sm:px-5 py-1 transition duration-150 hover:bg-[#FF4242] text-sm sm:text-base">
                            ...
                            <div>
                                <p class="text-[#595959] text-sm sm:text-base mb-3 mt-2">{{ $booking->container->residentialComplex->address }} Комплекс "{{ $booking->container->residentialComplex->title }}"</p>
                            </div>
                        </div>

                        <div class="flex flex-col sm:flex-row justify-between gap-4 sm:gap-6 mt-4">
                            <div class="flex flex-row sm:flex-col justify-between sm:justify-start gap-4 sm:gap-0">
                                <div>
                                    <h5 class="text-sm font-semibold sm:text-base">Дата начала аренды:</h5>
                                    <span class="text-[#595959] text-sm sm:text-base">{{ date('d.m.Y', strtotime($booking->start_date)) }}</span>
                                </div>
                                <div>
                                    <h5 class="text-sm font-semibold sm:text-base mt-0 sm:mt-2">Дата окончания аренды:</h5>
                                    <span class="text-[#595959] text-sm sm:text-base">{{ date('d.m.Y', strtotime($booking->end_date)) }}</span>
                                </div>
                            </div>
                            <div class="text-left sm:text-right">
                                <div>
                                    <h3 class="text-sm font-semibold sm:text-base">Стоимость контейнера:</h3>
                                    <span class="text-[#179BFF] text-lg sm:text-xl lg:text-2xl font-semibold">{{ number_format($booking->container->daily_price, 0, ',', ' ') }} ₽ / д.</span>
                                </div>
                                <div class="mt-2">
                                    <h3 class="text-sm font-semibold sm:text-base">Итоговая стоимость:</h3>
                                    <span class="text-[#179BFF] text-lg sm:text-xl lg:text-2xl font-semibold">{{ number_format($booking->total_price, 0, ',', ' ') }} ₽</span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    @else
        <div class="bg-white rounded-[20px] border-[1px] border-[#179BFF]/50 shadow-[5px_5px_15px_-5px_#71C2FF] p-8 sm:p-10 lg:p-12 text-center">
            ...
            <h4 class="text-[20px] sm:text-[22px] lg:text-[24px] font-semibold mb-2 sm:mb-3">У вас нет активных бронирований</h4>
            <p class="text-[#595959] text-base sm:text-lg mb-6 sm:mb-8">Вы еще не забронировали ни одного контейнера</p>
